...
import { buildAnalysisSystemPrompt, buildAnalysisUserPrompt, buildFixPrompt, buildASTContext } from './prompts.js';
...
export const analysisWorker = new Worker('analysis', async (job) => {
    ...
    try {
        ...
        const code = analysis.metadata?.sourceCode;
        if (!code) {
        ...
        const reportedIssues = args.issues || [];
        ...
        const rawDeductions = (reportedIssues.filter((i) => i.severity === 'error').length * 12 +
