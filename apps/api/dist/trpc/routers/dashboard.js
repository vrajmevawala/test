...
export const dashboardRouter = t.router({
    stats: workspaceProcedure
        ...
        const [statsRow] = await ctx.db
            ...
            .from(analyses)
            .where(and(eq(analyses.workspaceId, ctx.workspaceId), eq(analyses.status, 'complete'), gte(analyses.createdAt, since)));
        const [issueRow] = await ctx.db
            .select({
            totalIssues: count(issues.id),
            totalFixed: sum(sql `CASE WHEN ${issues.fixApplied} = true THEN 1 ELSE 0 END`),
        })
            .from(issues)
    ...
    weeklyChart: workspaceProcedure.query(async ({ ctx }) => {
        const rows = await ctx.db
            .select({
            day: sql `DATE(${analyses.createdAt})`.as('day'),
            count: count(analyses.id),
        })
            .from(analyses)
            .where(and(eq(analyses.workspaceId, ctx.workspaceId), gte(analyses.createdAt, new Date(Date.now() - 7 * 24 * 60 * 60 * 1000))))
    ...
    qualityScore: workspaceProcedure.query(async ({ ctx }) => {
        const [row] = await ctx.db
            ...
            .from(analyses)
            .where(and(eq(analyses.workspaceId, ctx.workspaceId), eq(analyses.status, 'complete'), gte(analyses.createdAt, new Date(Date.now() - 30 * 24 * 60 * 60 * 1000))));
        const categoryRows = await ctx.db
            .select({
            category: issues.category,
            total: count(issues.id),
            fixed: sum(sql `CASE WHEN ${issues.fixApplied} THEN 1 ELSE 0 END`),
        })
            .from(issues)
            .innerJoin(analyses, eq(issues.analysisId, analyses.id))
            .where(and(eq(analyses.workspaceId, ctx.workspaceId), gte(analyses.createdAt, new Date(Date.now() - 30 * 24 * 60 * 60 * 1000))))
            .groupBy(issues.category);
        return {
