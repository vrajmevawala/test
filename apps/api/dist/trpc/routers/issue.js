...
export const issueRouter = t.router({
    byAnalysis: workspaceProcedure
        ...
        return ctx.db
            .select()
            .from(issues)
            .where(and(eq(issues.analysisId, input.analysisId), ...(input.severity ? [eq(issues.severity, input.severity)] : []), ...(input.category ? [eq(issues.category, input.category)] : []), ...(input.fixableOnly ? [eq(issues.fixable, true)] : [])))
