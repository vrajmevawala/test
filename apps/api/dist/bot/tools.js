    ...
    switch (name) {
        case 'get_analysis': {
            const [row] = await db
                .select()
                .from(analyses)
                .where(and(eq(analyses.id, input.analysisId), eq(analyses.workspaceId, workspaceId)))
                .limit(1);
        ...
        case 'get_issues': {
            return db
                ...
                .where(and(eq(issues.analysisId, input.analysisId), eq(analyses.workspaceId, workspaceId), ...(input.severity ? [eq(issues.severity, input.severity)] : []), ...(input.category ? [eq(issues.category, input.category)] : []), ...(input.fixableOnly ? [eq(issues.fixable, true)] : [])))
        ...
        case 'apply_fix': {
            const [issue] = await db
                ...
                .from(issues)
                .innerJoin(analyses, eq(analyses.id, issues.analysisId))
                .where(and(eq(issues.id, input.issueId), eq(analyses.workspaceId, workspaceId)))
                .limit(1);
            if (!issue) {
